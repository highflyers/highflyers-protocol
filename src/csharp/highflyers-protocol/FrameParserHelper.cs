...
namespace HighFlyers.Protocol
{
    public class FrameParserHelper
    {
        ...
        {
            if (BitConverter.ToUInt32(bytes.ToArray(), bytes.Count - 5) !=
                Crc32.CalculateCrc32(bytes.Take(bytes.Count - 5).ToArray()))
        ...
        {
            if (endian == EndianType.Little && BitConverter.IsLittleEndian ||
                endian == EndianType.Big && !BitConverter.IsLittleEndian)
                return;
